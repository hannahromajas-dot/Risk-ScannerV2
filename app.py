...
def generate_sample_historical() -> pd.DataFrame:
    """Create realistic sample historical data so views work on first run."""
...
Business managers can identify recent risk trends in their specific industry and region (like manufacturing in Europe) so they can mitigate problems before it is too late.
    ...
    st.markdown(
        """
Investors can look at recent risk trends to see how a company or sector's risk levels are trending, helping them make smarter investment decisions.
        """
    )
...
* **Multi-Language News**: Expanding the app to scan business news written in other languages, for example - German financial reports.
    ...
    st.markdown("**Student Project by Hannah Romajas**")
    ...
    fig_bar.update_layout(
        ...
        xaxis=dict(title="Last 7 Days", tickangle=0),
